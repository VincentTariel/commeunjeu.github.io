...
\draw [->, >=latex,thick,color=black] (0,0) -- (1,2)node[above,left]{$\Vect{w}=\Vect{x}+\Vect{y}$};
\end{tikzpicture}  & $\begin{pmatrix}
1\\2
\end{pmatrix}=\begin{pmatrix}
2\\1
...
\draw [->,>=latex,thick] (0,0) -- node[above]{$0.5\Vect{x}$}(1,0.5);
\draw [->,>=latex,thick,] (1,0.5) --node[right]{$-\Vect{y}$} (2,-0.5);
\draw [->,>=latex,thick,color=black] (0,0) -- (2,-0.5)node[below]{$\Vect{w}=0.5\Vect{x}-\Vect{y}$};
\end{tikzpicture}  & $\begin{pmatrix}
2\\-0.5
\end{pmatrix}=0.5\begin{pmatrix}
2\\1
...
\end{tabular}
Il est très important de se représenter les espaces vectoriels, même les plus abstraits, comme des
...
\end{Exemple}
\begin{Exemple}[Familles de scalaires $\R^n$]
...
De même, l'ensemble des polynôme ${\K[X]}$ est aussi un espace vectoriel.\\
...
est appelé \defi{combinaison linéaire} des vecteurs $\Vect{x_1}, \Vect{x_2}, \ldots, \Vect{x_n}$.
...
Analyse :  on cherche un réel $\lambda$ tel que $(2,1)=\lambda (1,1)$,
ce qui équivaudrait à l'égalité $(2,1)=(\lambda, \lambda)$.  Ce système n'a aucune solution.\\
Conclusion : il n'existe pas $\lambda$ tel que $(2,1)=\lambda (1,1)$ donc $(2,1)$ n'est pas colinéaire au vecteur $(1,1)$.\\ 
\end{Correction}
\item \begin{Enonce}
    Démontrer que $
   ...
   \end{Enonce}
   \begin{Correction}
Analyse :  on cherche donc $\lambda$ et $\mu$ tel que  :
...
Synthèse : on vérifie que l'on a bien
...
\end{array}\right.$$
  ...
  Conclusion :  il n'existe pas $\lambda,\mu \in \R$ tels que $w=\lambda u + \mu v$.
\end{Correction}
\end{enumerate}
\end{Exemple}
...
Une autre solution est une étude du comportement asymptotique, c'est-à-dire $\exp$ tends vers l'infini en l'infini tandis que  $\lambda \cos + \mu \sin$ est bornée.\\
 Conclusion :  il n'existe pas $\lambda,\mu \in \R$ tels que $w=\lambda u + \mu v$ donc la fonction $\exp$ n'est pas une combinaison linéaire des fonctions  $\cos$ et $\sin$.
 \end{Correction}
\end{enumerate}
 \end{Exemple}
\begin{Exemple}[Matrices] 
Dans $\MnpK{2}{3}{\R}$, on considère
 $A=\begin{pmatrix}
 1&1&3\cr
 0&-1&4
...
\end{Remarque}
\begin{Methode}%label=methode,link=sous_espace_vectoriel
...
Par exemple,  $\K[X=\Vect(X^i)_{i\in \N}$.
\end{Remarque}
...
\begin{tikzpicture}[scale=1.3]
       ...
       \draw[->,>=latex,thick, colorprop] (-0.5,-0.5)--(-1.3,0.25) node[midway, below left] {$\Vect{y}$};
               \node[above left,colordef] at (2,-2) {$\text{Vect}(u,v)$};
...
De même l'intersection $F_1 \cap G \cap F_3 \cap \cdots \cap F_p$
d'une famille quelconque de sous-espaces vectoriels de $E$ est un sous-espace vectoriel.
\end{Proposition}

\begin{Demonstration}
\begin{itemize}
\item \impo{non vide :} comme $F$ et $G$ sont des sous-espaces vectoriels de $E$, $\Vect{0_E}\in F$ et $\Vect{0_E}\in G$ donc $\Vect{0_E}\subset F\cap G$
\item \impo{stable par $+$ :} oit $\Vect{x}_1,\Vect{x}_2 \in F\cap G $ d'où $\Vect{x}_1,\Vect{x}_2 \in F$ et $\Vect{x}_1,\Vect{x}_2 \in G$.\\
Comme $F$ est un sous-espace vectoriel, alors $\Vect{x}_1,\Vect{x}_2 \in F$ implique $\Vect{x}_1+\Vect{x}_2\in F$. De même,  $\Vect{x}_1,\Vect{x}_2 \in G$ implique $\Vect{x}_1+\Vect{x}_2\in G$. Donc $\Vect{x}_1+\Vect{x}_2\in F\cap G$.
\item
...
\end{Enonce}
\begin{Correction}
  l'intersection de deux plans vectoriels, $F$ et $G$ dans $\R^3$
\begin{center}
\begin{tikzpicture}
...
\end{Demonstration}
\begin{Remarque}
Pour démontrer que $F\cap G = \{\Vect{0_E}\}$, dans le raisonnement de la double inclusion, on omettra l'inclusion $F\cap G = \{\Vect{0_E}\}$ qui est évidente. En effet $\Vect{0_E}\in F,G$ car $F$ et $G$ sont des espaces vectoriels. 
\end{Remarque}
...
$$\begin{aligned}
 ...
 & \Leftrightarrow  (x,y,z) \in \VectE ( -1,0,1)
 \end{aligned}$$
...
$$\exists ! (Q,R) \in \K[X] \times \K_{n-1}[X]:\quad A = \overbrace{PQ}^{\in \K[X]}+ \overbrace{PQ}^{\in R}.$$
...
\begin{itemize}
 \item \impo{Analyse} : Supposons qu'il existe  $g\in\mathcal{P}$ et $h\in\mathcal{I}$ tel que $f=g+h$.\\
  ...
  \textbf{Synthèse.} Soit $x\in\R$.\\ 
  On pose $g,h$ définit par $$\forall x\in\R:\quad g(x)=\frac{f(x)+f(-x)}2 \text{ et }h(x)=\frac{f(x)-f(-x)}2.
  $$
  On a $f=g+h$ et d'autre part $g \in \mathcal{P}$ car $\forall x\in\R: g(-x)=g(x)$ et $h\in\mathcal{I}$  $\forall x\in\R: h(-x)=-h(x)$. D'où l'existence de la décomposition.	
\end{itemize}
...
\end{Proposition}
\begin{Demonstration}
La démonstration fait appel à des résultats ultérieurs (à visée pédagogique, cette proposition est placé ici).\\
...
Une famille finie  $(\Vect{x}_1,\dots,\Vect{x_p})$ est \defi{génératrice} de $E$ si tout vecteur de $E$ est combinaison linéaire de  $(\Vect{e}_1,\dots,\Vect{e_p})$, c'est à dire si $\VectE (\Vect{x}_1,\dots,\Vect{x_p}) = E$, c'est à dire si 
...
Par exemple, si on a $\sum_{i=0}^p a_i (X+1)^i=\sum_{i=0}^p i (X+1)^i$, alors $a_i=i\; \forall i\in\Intf{0}{p}$ car la famille $((X+1)^i)_{0\leq i\leq p }$ est libre. 
\end{Texte}
\begin{Exemple}
\begin{Enonce}
...
\end{Enonce}
\begin{Correction}
Comme  $(1,1)+ -2(0,1)=1(1,-1)$, un même vecteur s'exprime de manière différentes comme combinaison linéaire. Donc la famille n'est pas libre.
\end{Correction}
\end{Exemple}
...
\begin{Exemple}[Polynôme]
\begin{Enonce}
Démontrer que la famille $( (1-X),(1+3X-X^2))$ est liée.
\end{Enonce}
\begin{Correction}
...
$E$ est une et une seule manière combinaison linéaire de $\mathcal{B}$, c'est-à-dire,
...
\end{itemize}
La famille $(E_{ij})_{1\leqslant i\leqslant n, 1\leqslant j\leqslant p}$ est donc une base de $\MnK$.
\end{Demonstration}
...
\item Soit $P\in \K_n[X]$.\\
 ...
 Le polynôme $L-P$ s'annule aux $n+1$ points $X_i$. Comme   $L-P$ est de degré au plus $n$, $L-P=0$. D'où $L=P$. 
...
\item Soit $\lambda_0,\lambda_n\in \K$ tel que $\lambda_0 l_0+\lambda_1 l_1+\dots+\lambda_ nl_n =0$.
 Soit $i\in \Intf{0}{n}$. Comme $\lambda_0 l_0(x_i)+\lambda_1 l_1(x_i)+\dots+\lambda_i l_i(x_i)+\lambda_ nl_n(x_n)=\lambda_0 0+\lambda_1 0+\dots+\lambda_i 1+\lambda_n 0=\lambda_i$, $\lambda_i=0$. \\
...
\item Pour montrer que $\mathcal{B}$ est génératrice, il suffit de démontrer que tout vecteur de  $\mathcal{G}$ est combinaison linaire de $\mathcal{B}$.\\
...
\begin{enumerate}
  \item $P_1$:   on pose $\mathcal{B}_1 = \mathcal{B}_0 \cup \{ P_1 \} = \{ P_1 \}$.
  Comme  $P_1$ est non nul, $\mathcal{L}_1$ est une famille libre.

  \item $P_2$ : comme les éléments $P_1$ et $P_2$ sont linéairement indépendants,
  $\mathcal{L}_2 = \mathcal{L}_1\cup \{ P_2 \}= \{ P_1, P_2 \}$ est une famille libre.

  \item $P_3$ : Comme $P_3 = X+1 = P_1 + P_2)$, $(P_1, P_2, P_3 )$ est une famille liée.
  
  \item $P_4$ : on prouverait que les vecteurs $P_1$, $P_2$  et
...
Soit $(\Vect{v}_1,\Vect{v}_2)$ une famille de $E$. Il existe $\lambda_1$ et $\lambda_2$ dans $\R$ tel que $\Vect{v}_1=\lambda_1 \Vect{g}_1$ et $\Vect{v}_2=\lambda_2 \Vect{g}_1$. Par disjonction de cas, on a :
\begin{itemize}
...
Soit $E$ un espace vectoriel de dimension finie \propri{$n$}. Soit $\mathcal{F}$ est famille de \propri{$n$} vecteurs.
...
&\left\{ \begin{array}{rcl}
 ...
 \Leftrightarrow & 
  ...
  \end{array} \right.
  \end{aligned}$$
$t\neq 4$ si et seulement si  la seule solution est
...
\section{Théorème sur les  dimensions}
...
\end{Proposition}
\begin{Demonstration}
Soit $\mathcal{B}$ une base $E$. $\mathcal{B}$ est une famille génératrice de $E$. Donc, évidement de $F$ sous espace vectoriel de $E$.  En utilisant l'algorithme de base extraite, on peut construire une base $\mathcal{B}'$ de  $F$ en retirant certains vecteurs de $\mathcal{B}$. Donc la dimension de $F$ égale au cardinal de $\mathcal{B}'$ est inférieur au cardinal $\mathcal{B}$ égale à la dimension de $E$. Ainsi, $F$ est également de dimension finie et $\dim F \leq \dim E$
\end{Demonstration}

\begin{Remarque}
Si $E$ est de dimension infinie, on ne peut rien affirmer. Par exemple pour l'espace vectoriel $E = \SpaceFonction(\R,\R)$
de dimension infinie,  :
\begin{itemize}
  \item il contient le sous-espace vectoriel des fonctions polynômes de degré au plus $ n$, qui est de dimension finie ;
  \item et aussi le sous-espace vectoriel des fonctions polynômes, qui lui est de dimension infinie.
\end{itemize}
\end{Remarque}
\begin{Exemple}
...
\begin{itemize}
  ...
  les sous-espaces $\K \Vect{x} = \Vect( \Vect{x})$ engendrés par les vecteurs non nuls
  $\Vect{x}$ de $E$
  \item soit de dimension $2$ :  ce sont les plans vectorielles, c'est-à-dire
  les sous-espaces $\K \Vect{x}+\K \Vect{y} = \Vect ( \Vect{x},\Vect{y})$ engendrés par deux vecteurs non colinéaires de $E$
\end{itemize}
\end{Exemple}
...
On pose la famille $( (\Vect{e_1},\Vect{0}_F),\dots,(\Vect{e_n},\Vect{0}_F),(\Vect{0}_E,\Vect{f_1}),\dots,(\Vect{0}_E,\Vect{f_q}))$.
\begin{itemize}
...
Donc $\sum_{i=p+1}^{p+m} \mu_i\Vect{g_i}$ est égale  à un vecteur de $F$. Ainsi $\sum_{i=p+1}^{p+m} \mu_i\Vect{g_i}\in F\cap G$. Donc $\mu_i=0$ pour $i$ car les vecteurs $\Vect{g_{p+1}},\dots,\Vect{g_{p+m}}$ complètent la base $F\cap G$.\\
...
Comme le cardinal d'une base est égale à la dimension de l'espace vectoriel, on obtient  bien $$\dim(F+G) = \dim F + \dim G - \dim (F\cap G).$$
\end{Demonstration}
...
\begin{itemize}
\item \impo{(1) et (3) $\Rightarrow$ (2) :} d'après la formule de Grassmann $$\overbrace{\dim (F + G)}^{\dim E\text{ car } F+G=E} = \overbrace{\dim F + \dim G}^{\dim E \text{ d'après (3)}} - \dim(F \cap G),$$  on obtient  $\dim(F \cap G)=0$. Donc $F \cap G =\{\Vec{0_E}\}$.
\item \impo{(2) et (3) $\Rightarrow$ (1) :} d'après la formule de Grassmann
$$\overbrace{\dim (F + G)}^{\dim E\text{ car } F+G=E} = \dim F + \dim G - \overbrace{\dim(F \cap G)}^{=0\text{ car } F \cap G =\{\Vec{0_E}\}},$$
